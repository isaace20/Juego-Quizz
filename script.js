...
//arreglo que guardara la opcion correctas 
...
function cargarBandera(){
    //controlo si se acabaran las banderas
    if(banderas.length <= posActual){
        terminarJuego();
    }
    else{//carga las opciones
        //limpiamos las clases que se asignan
        limpiarOpciones();
...
function comprobarRespuesta(opElegida){
    if(opElegida==correcta[posActual]){//acerto
        ...
        document.getElementById('l' + correcta[posActual]).className = 'letra nombreAcertada';
    }
    posActual++;
